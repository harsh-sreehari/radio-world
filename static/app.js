...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const playingIcon = L.divIcon({
        className: 'radio-station-icon-playing',
        html: '<div></div>',
        iconSize: [16, 16]
    });
        ...
        .then(stations => {
            ...
            allStations.forEach(station => {
                if (station.geo_lat && station.geo_long) {
                    ...
                    stationMarkers[station.stationuuid] = marker; // Store marker by UUID
    ...
    function updateStationList(clickedStation) {
        ...
        if (nearbyStations.length === 0) {
            nearbyStations.push(clickedStation);
        }
